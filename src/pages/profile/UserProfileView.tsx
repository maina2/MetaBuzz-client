...
const UserProfileView: React.FC = () => {
  ...
  const [commentStates, setCommentStates] = useState<{[postId: string]: {
  ...
  if (profileError || followError) return <div className="text-center py-5 text-red-500">{profileError || followError}</div>;
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex justify-center md:justify-start space-x-6 mt-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-800">{selectedUser.followers_count}</p>
                <p className="text-xs text-gray-500 uppercase">Followers</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-800">{selectedUser.following_count}</p>
        ...
        {userPosts.length > 0 ? (
          userPosts.map((post) => (
            ...
            >
              ...
              <div className="flex items-center justify-around px-2 py-1 border-t border-b border-gray-100">
                <button 
                  ...
                  className={`flex items-center justify-center space-x-1 py-2 px-4 rounded-md transition-colors w-1/2 ${
                    likedPosts.includes(post.id) ? "text-red-500 bg-red-50" : "text-gray-600 hover:bg-gray-50"
                  }`}
                >
                  <svg 
                  ...
                  <span className="font-medium">{likedPosts.includes(post.id) ? 'Liked' : 'Like'}</span>
                </button>
                <button 
                  onClick={() => setCommentStates(prev => ({
                    ...prev,
                    [post.id]: { 
                      ...prev[post.id], 
                  ...
                  className={`flex items-center justify-center space-x-1 py-2 px-4 rounded-md transition-colors w-1/2 ${
                    commentStates[post.id]?.isCommenting ? "text-blue-500 bg-blue-50" : "text-gray-600 hover:bg-gray-50"
                  }`}
                >
              ...
              {/* Comments Section */}
              <div className={`px-5 py-3 transition-all duration-300 ${commentStates[post.id]?.isCommenting ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                {/* Comments List */}
                {!post.comments || post.comments.length === 0 ? (
                  ...
                  <div className="flex items-start space-x-2 mt-3">
                    ...
                    <div className="flex-1 bg-gray-50 rounded-2xl p-1 pl-3 flex items-center">
                      ...
                      <button
                        ...
                        disabled={!commentStates[post.id]?.newComment?.trim() || commentStates[post.id]?.isSubmitting}
                        className={`p-2 rounded-full transition-all ${
                          !commentStates[post.id]?.newComment?.trim() || commentStates[post.id]?.isSubmitting 
