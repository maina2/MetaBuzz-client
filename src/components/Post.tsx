    ...
    <div className="bg-white rounded-xl shadow-md mb-6 overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-100">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <Link to={`/profile/${post.user}`} className="font-semibold text-gray-800 hover:text-teal-500 transition-colors">
              {post.user}
            </Link>
      ...
      <div className="flex items-center justify-between px-5 py-2 text-sm text-gray-500 border-t border-gray-100">
        <div className="flex items-center space-x-2">
          ...
          <span>{likes.length > 0 ? `${likes.length} ${likes.length === 1 ? 'like' : 'likes'}` : 'Be the first to like'}</span>
        </div>
        <div>
          {memoizedComments.length > 0 && (
            <span>{memoizedComments.length} {memoizedComments.length === 1 ? 'comment' : 'comments'}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-around px-2 py-1 border-t border-b border-gray-100">
        <button 
          ...
          className={`flex items-center justify-center space-x-1 py-2 px-4 rounded-md transition-colors w-1/2 ${
            isLikedByUser ? "text-red-500 bg-red-50" : "text-gray-600 hover:bg-gray-50"
          }`}
        >
          <svg 
          ...
          <span className="font-medium">{isLikedByUser ? 'Liked' : 'Like'}</span>
        </button>
        <button 
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <Link to={`/profile/${comment.user}`} className="font-medium text-gray-800 hover:text-blue-500 transition-colors text-sm">
                        {comment.user}
                      </Link>
